...
function uninstall_cp4ba(){
    printf "\n"
    printf "\x1B[1mUninstall CP4BA Operator...\n\x1B[0m"
...
function uninstall_olm_cp4ba(){
    ...
    csvName=$(kubectl get subscription "ibm-cp4ba-operator-catalog-subscription" -n $NAMESPACE -o go-template --template '{{.status.installedCSV}}')
