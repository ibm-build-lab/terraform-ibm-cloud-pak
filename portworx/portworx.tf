...
data "ibm_container_vpc_cluster_worker" "this" {
  ...
  resource_group_id = data.ibm_resource_group.group.id
  worker_id         = data.ibm_container_vpc_cluster.this[0].workers[count.index]
}
...
resource "ibm_is_volume" "this" {
  ...
  name = "${var.unique_id}-pwx-${split("-", data.ibm_container_vpc_cluster.this[0].workers[count.index])[4]}"
