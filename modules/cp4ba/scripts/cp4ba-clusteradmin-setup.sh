...
#eval "$(jq -r '@sh "export PROJ_NAME=\(.Project_Name)"')"
#SELECTED=""




#echo "{\"output\":\"${PLATFORM_SELECTED}" "${DEPLOYMENT_TYPE}" "${PLATFORM_VERSION}" "${PROJ_NAME}\"}"


# Assign the project name

#echo "{\"output\":\"$PROJ_NAME\"}"

# Assign the platform version
#eval "$(jq -r '@sh "export PLATFORM_VERSION=\(.Platform_Version)"')"
#echo "{\"output\":\"$PLATFORM_VERSION\"}"


#eval "$(jq -r '@sh "export KUBECONFIG=\(.kubeconfig) CP4BA_NAMESPACE=\(.namespace) "')"

#echo "{\"output\":\"$PLATFORM_SELECTED \n $PROJ_NAME \n $PLATFORM_VERSION\"}"

#echo "{\"output\":\"$KUBECONFIG\"}"
#echo "(\"output\":\"$cp4ba_namespace\")"
...
function select_platform(){
    printf "\n"
    COLUMNS=12
#    eval "$(jq -r '@sh "export options=\(.option)"')"
...
#    then
#        option_types=("Enterprise")
#        PS3='Enter a valid option [1 to 1]: '
#        select opt in "${option_types[@]}"
#        do
#            case $opt in
#                "Enterprise")
#                    DEPLOYMENT_TYPE="enterprise"
...
########################## Assigning the project name or namespace
function collect_input() {
  # Done
#  eval "$(jq -r '@sh "export project_name=\(.project_name)"')"
#  eval "$(jq -r '@sh "export KUBECONFIG=\(.kubeconfig) project_name=\(.project_name)"')"
    project_name=""
...
function prepare_olm_install() {
   ...
   for ((retry=0;retry<=${maxRetry};retry++)); do
      ...
      isReady=$(${CLI_CMD} get pod -n "$project_name" --no-headers | grep ibm-cp4ba-operator | grep "Running")
      if [[ -z $isReady ]]; then
        if [[ $retry -eq ${maxRetry} ]]; then
...
#        read -rp "" ans
#        eval "$(jq -r '@sh "export entitlement_key=\(.hasEntitlementKey)"')"
...
#            do
#                eval "$(jq -r '@sh "export entitlement_key=\(.CP4BA_ENTITLED_REGISTRY_KEY)"')"
...
#        printf "\x1B[1mTo provision the persistent volumes and volume claims: \x1B[0m"

#        while [[ $storage_class_name == "" ]]
#        do
#            eval "$(jq -r '@sh "export storage_class_name=\(.Storage_Class_Name)"')"
...
#            eval "$(jq -r '@sh "export storage_class_name=\(.SC_Fast_Storage_Class_Name)"')"
...
function select_ocp_olm(){
    ...
    while true; do
        ...
        eval "$(jq -r '@sh "export ans=\(.OCP_Catalog)"')"
#        read -rp "" ans
        case "$ans" in
        "y"|"Y"|"yes"|"Yes"|"YES")
            SCRIPT_MODE="OLM"
...
#            read -rp "" local_public_registry_server
            eval "$(jq -r '@sh "export local_public_registry_server=\(.Local_Public_Registry_Server)"')"
...
#        read -rp "" local_registry_server
        eval "$(jq -r '@sh "export local_registry_server=\(.Local_Registry_Server)"')"
...
#       eval "$(jq -r '@sh "export local_registry_user=\(.CP4BA_ENTITLED_REGISTRY_USER)"')"
...
function get_local_registry_password(){
    ...
    do
#      eval "$(jq -r '@sh "export local_registry_password=\(.CP4BA_ENTITLED_REGISTRY_KEY)"')"
