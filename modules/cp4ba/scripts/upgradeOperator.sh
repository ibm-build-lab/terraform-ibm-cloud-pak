...
PLATFORM_SELECTED=$(eval echo $(kubectl get icp4bacluster $(kubectl get icp4bacluster -n $NAMESPACE | grep NAME -v | awk '{print $1}') -n $NAMESPACE -o yaml | grep sc_deployment_platform | tail -1 | cut -d ':' -f 2))
...
function prepare_olm_install() {
   ...
   for ((retry=0;retry<=${maxRetry};retry++)); do        
      ...
      isReady=$(oc get pod -n "$project_name" --no-headers | grep ibm-cp4ba-operator | grep "Running")
      if [[ -z $isReady ]]; then
        if [[ $retry -eq ${maxRetry} ]]; then 
...
function uninstall_cp4ba(){
    printf "\n"
    printf "\x1B[1mUnDeploying the previous CP4BA Operator...\n\x1B[0m"
...
function uninstall_olm_cp4ba(){
    ...
    csvName=$(oc get subscription "ibm-cp4ba-operator" -o go-template --template '{{.status.installedCSV}}')
...
function create_new_shared_logs_pvc(){
    if [[ $(kubectl get icp4bacluster) == '' ]]; then
        echo -e "\033[31mIf you don't have a CR deployed, we can't upgrade CP4BA Operator only, pls run deleteOperator.sh and then deployOperator.sh to redeploy Operator.\033[0m"
        exit 1
    fi
    DEPLOYMENT_TYPE=$(eval echo $(kubectl get icp4bacluster $(kubectl get icp4bacluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_deployment_type | tail -1 | cut -d ':' -f 2))
    STORAGE_CLASS_NAME=$(eval echo $(kubectl get icp4bacluster $(kubectl get icp4bacluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_dynamic_storage_classname | tail -1 | cut -d ':' -f 2))
    SLOW_STORAGE_CLASS_NAME=$(eval echo $(kubectl get icp4bacluster $(kubectl get icp4bacluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_slow_file_storage_classname | tail -1 | cut -d ':' -f 2))
    FAST_STORAGE_CLASS_NAME=$(eval echo $(kubectl get icp4bacluster $(kubectl get icp4bacluster | grep NAME -v | awk '{print $1}') -o yaml | grep sc_fast_file_storage_classname | tail -1 | cut -d ':' -f 2))
...
function cp4ba_operator_uninstall(){
  if [[ $(kubectl get deployment -n $NAMESPACE --no-headers | grep ibm-cp4ba-operator | awk '{print $1}') != 0 ]]; then
    kubectl delete deployment $(kubectl get deployment -n $NAMESPACE --no-headers | grep ibm-cp4ba-operator | awk '{print $1}') -n $NAMESPACE
  fi

  local maxRetry=20
  for ((retry=0;retry<=${maxRetry};retry++)); do        
      echo "Waiting for CP4BA operator pod to be removed...."
       
      isReady=$(oc get pod -n "$NAMESPACE" | grep ibm-cp4ba-operator )
      if [[ -z $isReady ]]; then
        echo "CP4BA operator deleted!"
        break
      else
        if [[ $retry -eq ${maxRetry} ]]; then 
          echo -e "\x1B[1;31mTimeout Waiting for CP4BA operator to be removed!\n\x1B[0m"
          exit 1
        else
          sleep 30
          continue
...
if [[ $LICENSE_ACCEPTED == "accept" ]]; then
  if [[ $(kubectl get pvc | grep cp4ba-shared-log-pvc) == '' ]]; then
  ...
  if [[ "$PLATFORM_SELECTED" == "OCP" || "$PLATFORM_SELECTED" == "ocp" || "$PLATFORM_SELECTED" == "ROKS" || "$PLATFORM_SELECTED" == "roks" ]]; then
    kubectl get subscription -n $NAMESPACE| grep ibm-cp4ba-operator-catalog-subscription >/dev/null 2>&1
    returnValue=$?
    if [ "$returnValue" == 1 ] ; then
        uninstall_cp4ba
    fi
    # cp4ba_operator_uninstall
    # uninstall_cp4ba
    prepare_olm_install
  else
    cncf_install
