...
OPERATOR_IMAGE=${DOCKER_REG_SERVER}/cp/cp4a/icp4a-operator:21.0.1
...
function prompt_license(){
    ...
    if [[ $retVal_baw -eq 1 ]]; then
        ...
        echo -e "\x1B[1;31mhttps://github.com/icp4a/cert-kubernetes/blob/21.0.1/LICENSE\n\x1B[0m"
        INSTALL_BAW_ONLY="No"
    fi
        ...
        while true; do
            case $item_pattern in
                ...
                "IBM Automation Document Processing")
                    if [[ $DEPLOYMENT_TYPE == "demo" ]]; then
                        optional_components_list=("Content Search Services" "Content Management Interoperability Services")
                        optional_components_cr_list=("css" "cmis")
                        show_optional_components
...
function verify_local_registry_password(){
    # require to preload image for CP4BA image and ldap/db2 image for demo
    printf "\n"
    while true; do
...
function enable_ae_data_persistence_baa(){
    ...
    then
        ...
        content_start="$(grep -n "The beginning section of database configuration for CP4BA" ${APPLICATION_PATTERN_FILE_TMP} | head -n 1 | cut -d: -f1)"
...
function set_ldap_type_foundation(){
    if [[ $DEPLOYMENT_TYPE == "enterprise" ]] ;
    then
        # ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_BAK} ${CP4BA_PATTERN_FILE_TMP}

        if [[ "$LDAP_TYPE" == "AD" ]]; then
            content_start="$(grep -n "ad:" ${CP4BA_PATTERN_FILE_TMP} | head -n 1 | cut -d: -f1)"
        else
            content_start="$(grep -n "tds:" ${CP4BA_PATTERN_FILE_TMP} | head -n 1 | cut -d: -f1)"
        fi
        content_stop="$(tail -n +$content_start < ${CP4BA_PATTERN_FILE_TMP} | grep -n "lc_group_filter:" | head -n1 | cut -d: -f1)"
        content_stop=$(( $content_stop + $content_start - 1))
        vi ${CP4BA_PATTERN_FILE_TMP} -c ':'"${content_start}"','"${content_stop}"'s/    # /    ' -c ':wq' >/dev/null 2>&1

        # ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_PATTERN_FILE_BAK}
    fi
}
...
function input_information(){
    ...
    then
        existing_infra_name=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.sc_deployment_hostname_suffix`
        chrlen=${#existing_infra_name}
        INFRA_NAME=${existing_infra_name:21:chrlen}
        existing_ldap_type=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.ldap_configuration.lc_selected_ldap_type`
        ...
        existing_docker_reg_server=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.sc_image_repository`
        if [[ "$existing_docker_reg_server" == *"icr.io"* ]]; then
            use_entitlement="yes"
        fi

        local_registry_server=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.sc_image_repository`
        ...
        IFS=$OIFS
        CONVERT_LOCAL_REGISTRY_SERVER=${joined}
        DOCKER_RES_SECRET_NAME=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.image_pull_secrets.[0]`
        STORAGE_CLASS_NAME=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.storage_configuration.sc_dynamic_storage_classname`
        SLOW_STORAGE_CLASS_NAME=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.storage_configuration.sc_slow_file_storage_classname`
        MEDIUM_STORAGE_CLASS_NAME=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.storage_configuration.sc_medium_file_storage_classname`
        FAST_STORAGE_CLASS_NAME=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.shared_configuration.storage_configuration.sc_fast_file_storage_classname`
    fi
    ...
    if [[ $IBM_LICENS == "Accept" ]]; then
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.ibm_license "accept"
    else
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.ibm_license ""
    fi
}
...
function apply_cp4ba_operator(){
    ...
    ATTEMPTS=0
    ROLLOUT_STATUS_CMD="${CLI_CMD} rollout status deployment/ibm-cp4ba-operator"
...
function copy_jdbc_driver(){
    ...
    operator_podname=$(${CLI_CMD} get pod|grep ibm-cp4ba-operator|grep Running|awk '{print $1}')
...
function set_foundation_components(){
    ...
    if (( ${#FOUNDATION_DELETE_LIST[@]} > 0 ));then
        ...
        for item in "${FOUNDATION_DELETE_LIST[@]}"; do
            if [[ "$item" == "BAS" ]];then
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.bastudio_configuration
            fi
            if [[ "$item" == "UMS" ]];then
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.ums_configuration
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.datasource_configuration.dc_ums_datasource
            fi
            if [[ "$item" == "BAN" ]];then
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.navigator_configuration
            fi
            if [[ "$item" == "RR" ]];then
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.resource_registry_configuration
            fi
            if [[ "$item" == "AE" ]];then
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.application_engine_configuration
            fi
        done
    fi
    # ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_PATTERN_FILE_BAK}
...
function merge_pattern(){
        ...
        while true; do
            case $item in
                "content")
                    ...
                    ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${CONTENT_PATTERN_FILE_BAK}
                    break
                    ;;
                "contentanalyzer")
                    set_aca_tenant_pattern
                    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.datasource_configuration.dc_ca_datasource.tenant_databases
                    ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${ACA_PATTERN_FILE_BAK}
                    break
                    ;;
                "decisions")
                    set_decision_feature
                    ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${DECISIONS_PATTERN_FILE_BAK}
                    break
                    ;;
                "workflow")
                    # set_ldap_type_workflow_pattern
                    if [[ "${INSTALL_BAW_ONLY}" == "Yes" ]]; then
                        # ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.baw_configuration
                        if [[ $DEPLOYMENT_TYPE == "enterprise" ]];then
                            ...
                            # fi
                            ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${WORKFLOW_PATTERN_FILE_BAK}
                ...
                "workflow-authoring")
                    ...
                    if [[ $DEPLOYMENT_TYPE == "enterprise" ]];then
                        ...
                        # fi
                        ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${WORKFLOW_AUTHOR_PATTERN_FILE_BAK}
                    fi
                    break
                    ;;
                "workflow-runtime")
                ...
                "workstreams")
                    ...
                    if [[ " ${PATTERNS_CR_SELECTED[@]} " =~ "workstreams" && " ${PATTERNS_CR_SELECTED[@]} " =~ "workflow-runtime" ]]; then
                        ...
                        # fi
                        ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${WORKSTREAMS_PATTERN_FILE_BAK}
                    fi
                    break
                    ;;
                "workflow-workstreams")
                    ...
                    if [[ $DEPLOYMENT_TYPE == "enterprise" ]];then
                        if [[ $INSTALLATION_TYPE == "existing" ]]; then
                            ...
                            #     ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${WORKFLOW_PATTERN_FILE_BAK}
                            # fi
                            ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${WW_PATTERN_FILE_BAK}
                        else
                            ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${WW_PATTERN_FILE_BAK}
                ...
                "application")
                    ...
                    ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${APPLICATION_PATTERN_FILE_BAK}
                    break
                    ;;
                "digitalworker")
                    ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${ADW_PATTERN_FILE_BAK}
                    break
                    ;;
                "decisions_ads")
                    set_ads_designer_runtime
                    ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${ADS_PATTERN_FILE_BAK}
                    break
                    ;;
                "document_processing")
                    set_aria_gpu
                    ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${ARIA_PATTERN_FILE_BAK}
                    break
                    ;;
    ...
    # ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.image_pull_secrets
    # ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.image_pull_secrets.[0] "image-pull-secret"
    # if [[ (" ${EXISTING_PATTERN_ARR[@]} " =~ "workstreams") && !(" ${PATTERNS_CR_SELECTED[@]} " =~ "workstreams") ]]; then
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.baw_configuration
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.elasticsearch_configuration
    # fi
    # if [[ (" ${EXISTING_PATTERN_ARR[@]} " =~ "workflow-runtime") && !(" ${PATTERNS_CR_SELECTED[@]} " =~ "workflow-runtime") ]]; then
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.baw_configuration
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.baml_configuration
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.elasticsearch_configuration
    # fi

    # if [[ (" ${EXISTING_PATTERN_ARR[@]} " =~ "workflow-authoring") && !(" ${PATTERNS_CR_SELECTED[@]} " =~ "workflow-authoring") ]]; then
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.workflow_authoring_configuration
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.elasticsearch_configuration
    # fi
    ...
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.elasticsearch_configuration
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.baml_configuration
    #     ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.pfs_configuration
    # fi
    # ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_PATTERN_FILE_BAK}
    ...
    #     ${YQ_CMD} d -i ${CP4BA_EXISTING_TMP} spec.datasource_configuration.dc_os_datasources
    #     ${YQ_CMD} d -i ${CP4BA_EXISTING_TMP} spec.initialize_configuration
    # fi
        ...
        while true; do
            case $item in
                ...
                "ums")
                    ...
                    ;;
                "cmis")
                    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.ecm_configuration.cmis
                    break
                    ;;
                "css")
                    break
                    ;;
                "es")
                    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.ecm_configuration.es
                    break
                    ;;
                "tm")
                    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.ecm_configuration.tm
                    break
                    ;;
                "ier")
                    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.ier_configuration
                    break
                    ;;
                "iccsap")
                    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.iccsap_configuration
                    break
                    ;;
                "bai")
                    if [[ (" ${PATTERNS_CR_SELECTED[@]} " =~ "workflow-runtime") && (" ${PATTERNS_CR_SELECTED[@]} " =~ "workstreams") ]]; then
                        ...
                        ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.bai_configuration
                        break
                    fi
                    ;;
                "ads_designer")
...
function get_existing_pattern_name(){
    ...
    # printf "\x1B[1mDefault is \x1B[0m(${CP4BA_PATTERN_FILE_BAK}): "
    # existing_pattern_cr_name=`${CLI_CMD} get icp4bacluster|awk '{if(NR>1){if(NR==2){ arr=$1; }else{ arr=arr" "$1; }} } END{ print arr }'`

    while [[ $existing_pattern_cr_name == "" ]];
    do
        read -p "[Default=$CP4BA_PATTERN_FILE_BAK]: " existing_pattern_cr_name
        : ${existing_pattern_cr_name:=$CP4BA_PATTERN_FILE_BAK}
        if [ -f "$existing_pattern_cr_name" ]; then
            ...
            FOUNDATION_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOYMENT_TYPE}_foundation.yaml
            if [[ "$existing_pattern_cr_name" == "$CP4BA_PATTERN_FILE_BAK" ]]; then
                ${COPY_CMD} -rf "${CP4BA_PATTERN_FILE_BAK}" "${CP4BA_EXISTING_BAK}"
                ${COPY_CMD} -rf "${CP4BA_PATTERN_FILE_BAK}" "${CP4BA_EXISTING_TMP}"
            else
                ${COPY_CMD} -rf "${existing_pattern_cr_name}" "${CP4BA_PATTERN_FILE_BAK}"
                ${COPY_CMD} -rf "${existing_pattern_cr_name}" "${CP4BA_EXISTING_BAK}"
                ${COPY_CMD} -rf "${existing_pattern_cr_name}" "${CP4BA_EXISTING_TMP}"
            fi
            # ${COPY_CMD} -rf "${FOUNDATION_PATTERN_FILE}" "${CP4BA_PATTERN_FILE_TMP}"
            # ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_PATTERN_FILE_BAK}
            # ${COPY_CMD} -rf "${CP4BA_PATTERN_FILE_BAK}" "${CP4BA_PATTERN_FILE_TMP}"
        else
    ...
    # existing_pattern_list=`${CLI_CMD} get icp4bacluster $existing_pattern_cr_name -o yaml | yq r - spec.shared_configuration.sc_deployment_patterns`
    # existing_pattern_deploy_type=`${CLI_CMD} get icp4bacluster $existing_pattern_cr_name -o yaml | yq r - spec.shared_configuration.sc_deployment_type`
...
function apply_pattern_cr(){
    # echo -e "\x1B[1mCreating a custom resource YAML file for IBM CP4BA Operator ......\x1B[0m"
    # echo "length of optional_component_cr_arr:${#optional_component_cr_arr[@]}"
    # echo "!!optional_component_cr_arr!!!${optional_component_cr_arr[*]}"
    # echo "EXISTING_PATTERN_ARR: ${EXISTING_PATTERN_ARR[*]}"
    # echo "PATTERNS_CR_SELECTED: ${PATTERNS_CR_SELECTED[*]}"
    ...
    # ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_BAK} ${CP4BA_PATTERN_FILE_TMP}
    # remove merge issue
    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} metadata.labels.app.*
    # if [[ $INSTALLATION_TYPE == "existing" ]]; then
    #     ae_instance=`cat $CP4BA_EXISTING_BAK | ${YQ_CMD} r - spec.application_engine_configuration.[0].name`
    ...
    if [[ "${INSTALLATION_TYPE}" == "existing" ]]; then
        ...
        ${YQ_CMD} d -i ${CP4BA_EXISTING_TMP} spec.shared_configuration.sc_deployment_patterns
        ${YQ_CMD} d -i ${CP4BA_EXISTING_TMP} spec.shared_configuration.sc_optional_components
        ${SED_COMMAND} '/tag: /d' ${CP4BA_EXISTING_TMP}
        ${SED_COMMAND} '/appVersion: /d' ${CP4BA_EXISTING_TMP}
        ${SED_COMMAND} '/release: /d' ${CP4BA_EXISTING_TMP}
        # ${YQ_CMD} m -a -i -M ${CP4BA_EXISTING_BAK} ${CP4BA_PATTERN_FILE_TMP}
        # ${COPY_CMD} -rf ${CP4BA_EXISTING_BAK} ${CP4BA_PATTERN_FILE_TMP}
        # ${YQ_CMD} m -a -i -M ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_EXISTING_BAK}
        # read -rsn1 -p"After Merge: Press any key to continue";echo
    fi

    ${SED_COMMAND_FORMAT} ${CP4BA_PATTERN_FILE_TMP}
    # ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_PATTERN_FILE_BAK}
    ...
    if [[ $INSTALLATION_TYPE == "existing" ]]; then
        ...
        if (( ${#object_array[@]} >= 1 ));then
            for object_name in "${object_array[@]}"
            do
                containsObjectStore "$object_name" "${CP4BA_EXISTING_TMP}"
                if (( ${#os_index_array[@]} >= 1 ));then
                    # ((index_array_temp=${#os_index_array[@]}-1))
                    for ((j=0;j<${#os_index_array[@]};j++))
                    do
                        ((index_os=${os_index_array[$j]}-j))
                        ${YQ_CMD} d -i ${CP4BA_EXISTING_TMP} spec.datasource_configuration.dc_os_datasources.[$index_os]
                    done
                fi
                containsInitObjectStore "$object_name" "${CP4BA_EXISTING_TMP}"
                if (( ${#os_index_array[@]} >= 1 ));then
                    # ((index_array_temp=${#os_index_array[@]}-1))
                    for ((j=0;j<${#os_index_array[@]};j++))
                    do
                        ((index_os=${os_index_array[$j]}-j))
                        ${YQ_CMD} d -i ${CP4BA_EXISTING_TMP} spec.initialize_configuration.ic_obj_store_creation.object_stores.[$index_os]
                    done
                fi
            done
            object_array=()
        ...
        if [[ (" ${EXISTING_PATTERN_ARR[@]} " =~ "content") && (" ${PATTERNS_CR_SELECTED[@]} " =~ "content") ]]; then
            ...
            getTotalFNCMObjectStore "${CP4BA_PATTERN_FILE_TMP}"
            ...
            getTotalFNCMObjectStore "${CP4BA_EXISTING_TMP}"
        ...
        if grep "ums_configuration:" $CP4BA_EXISTING_TMP > /dev/null
        then
            ${YQ_CMD} w -i ${CP4BA_EXISTING_TMP} spec.ums_configuration.fix "dummy"
        fi
        # read -rsn1 -p"Before:Press any key to exit";echo
        ${YQ_CMD} m -i -a -M --overwrite --autocreate=false ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_EXISTING_TMP}
        # read -rsn1 -p"After:Press any key to exit";echo
        ${YQ_CMD} d -i ${CP4BA_EXISTING_TMP} spec.ums_configuration.fix
        ${SED_COMMAND} "s|ums_configuration: {}|ums_configuration:|g" ${CP4BA_EXISTING_TMP}
        ${SED_COMMAND} "s|ums_configuration: {}|ums_configuration:|g" ${CP4BA_PATTERN_FILE_TMP}
    fi

    # ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_BAK} ${CP4BA_PATTERN_FILE_TMP}
    if [[ " ${OPT_COMPONENTS_CR_SELECTED[@]} " =~ "ae_data_persistence" ]]; then
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.sc_content_initialization "true"
    fi

    if [[ "$CPE_FULL_STORAGE" == "Yes" ]]; then
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.sc_cpe_limited_storage "false"
    elif [[ "$CPE_FULL_STORAGE" == "No" ]]; then
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.sc_cpe_limited_storage "true"
    else
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.sc_cpe_limited_storage "false"
    fi

    # Set sc_deployment_patterns
    ${SED_COMMAND} "s|sc_deployment_patterns:.*|sc_deployment_patterns: \"$pattern_joined\"|g" ${CP4BA_PATTERN_FILE_TMP}

    # Set sc_optional_components='' when none optional component selected
    if [ "${#optional_component_cr_arr[@]}" -eq "0" ]; then
        ${SED_COMMAND} "s|sc_optional_components:.*|sc_optional_components: \"\"|g" ${CP4BA_PATTERN_FILE_TMP}
    else
        ${SED_COMMAND} "s|sc_optional_components:.*|sc_optional_components: \"$opt_components_joined\"|g" ${CP4BA_PATTERN_FILE_TMP}
    fi

    # Set sc_deployment_platform
    ${SED_COMMAND} "s|sc_deployment_platform:.*|sc_deployment_platform: \"$PLATFORM_SELECTED\"|g" ${CP4BA_PATTERN_FILE_TMP}

    # Set sc_deployment_type
    ${SED_COMMAND} "s|sc_deployment_type:.*|sc_deployment_type: \"$DEPLOYMENT_TYPE\"|g" ${CP4BA_PATTERN_FILE_TMP}


    # Set sc_deployment_hostname_suffix
    if  [[ $PLATFORM_SELECTED == "OCP" || $PLATFORM_SELECTED == "ROKS" ]];
    then
        ${SED_COMMAND} "s|sc_deployment_hostname_suffix:.*|sc_deployment_hostname_suffix: \"{{ meta.namespace }}.${INFRA_NAME}\"|g" ${CP4BA_PATTERN_FILE_TMP}
    else
        ${SED_COMMAND} "s|sc_deployment_hostname_suffix:.*|sc_deployment_hostname_suffix: \"{{ meta.namespace }}\"|g" ${CP4BA_PATTERN_FILE_TMP}
    fi
    ...
    if [[ $DEPLOYMENT_TYPE == "enterprise" ]];then
        if [[ $LDAP_TYPE == "AD" ]];then
            # ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.ldap_configuration.lc_selected_ldap_type "\"Microsoft Active Directory\""
            ${SED_COMMAND} "s|lc_selected_ldap_type:.*|lc_selected_ldap_type: \"Microsoft Active Directory\"|g" ${CP4BA_PATTERN_FILE_TMP}

        elif [[ $LDAP_TYPE == "TDS" ]]
        then
            # ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.ldap_configuration.lc_selected_ldap_type "IBM Security Directory Server"
            ${SED_COMMAND} "s|lc_selected_ldap_type:.*|lc_selected_ldap_type: \"IBM Security Directory Server\"|g" ${CP4BA_PATTERN_FILE_TMP}
        fi
    fi
    # Set sc_dynamic_storage_classname
    if [[ "$PLATFORM_SELECTED" == "ROKS" ]]; then
        ${SED_COMMAND} "s|sc_dynamic_storage_classname:.*|sc_dynamic_storage_classname: ${FAST_STORAGE_CLASS_NAME}|g" ${CP4BA_PATTERN_FILE_TMP}
    else
        ${SED_COMMAND} "s|sc_dynamic_storage_classname:.*|sc_dynamic_storage_classname: ${STORAGE_CLASS_NAME}|g" ${CP4BA_PATTERN_FILE_TMP}
    fi
    ${SED_COMMAND} "s|sc_slow_file_storage_classname:.*|sc_slow_file_storage_classname: ${SLOW_STORAGE_CLASS_NAME}|g" ${CP4BA_PATTERN_FILE_TMP}
    ${SED_COMMAND} "s|sc_medium_file_storage_classname:.*|sc_medium_file_storage_classname: ${MEDIUM_STORAGE_CLASS_NAME}|g" ${CP4BA_PATTERN_FILE_TMP}
    ${SED_COMMAND} "s|sc_fast_file_storage_classname:.*|sc_fast_file_storage_classname: ${FAST_STORAGE_CLASS_NAME}|g" ${CP4BA_PATTERN_FILE_TMP}
    # Set image_pull_secrets
    # ${SED_COMMAND} "s|image-pull-secret|$DOCKER_RES_SECRET_NAME|g" ${CP4BA_PATTERN_FILE_TMP}
    ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.image_pull_secrets
    ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.image_pull_secrets.[0] "$DOCKER_RES_SECRET_NAME"

    # set sc_image_repository
    if [ "$use_entitlement" = "yes" ] ; then
        ${SED_COMMAND} "s|sc_image_repository:.*|sc_image_repository: ${DOCKER_REG_SERVER}|g" ${CP4BA_PATTERN_FILE_TMP}
    else
        ${SED_COMMAND} "s|sc_image_repository:.*|sc_image_repository: ${CONVERT_LOCAL_REGISTRY_SERVER}|g" ${CP4BA_PATTERN_FILE_TMP}
    fi

    #
    if [[ "$ENABLE_GPU_ARIA" == "Yes" ]]; then
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.node_labels.gpu_enabled "true"
        # ${YQ_CMD} w -i ${ARIA_PATTERN_FILE_TMP} spec.ca_configuration.deeplearning.nodelabel_key "$nodelabel_key"
        ${SED_COMMAND} "s|gpu_nodelabel_key:.*|gpu_nodelabel_key: \"$nodelabel_key\"|g" ${CP4BA_PATTERN_FILE_TMP}
    elif [[ "$ENABLE_GPU_ARIA" == "No" ]]
    then
        ${YQ_CMD} w -i ${CP4BA_PATTERN_FILE_TMP} spec.shared_configuration.node_labels.gpu_enabled "false"
    fi
    # Replace image URL
    ...
    if [ "$use_entitlement" = "yes" ] ; then
        ${SED_COMMAND} "s/$REGISTRY_IN_FILE/$DOCKER_REG_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
    else
        ${SED_COMMAND} "s/$old_db2/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_db2_alpine/$CONVERT_LOCAL_REGISTRY_SERVER\/alpine/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_ldap/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_db2_etcd/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_busybox/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_demo/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_fmcn/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_ban/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_ums/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_bas/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_aae/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_baca/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_odm/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_baw/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_iaws/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CPBA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_ads/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_workflow/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_adp/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_ier/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "s/$old_iccsap/$CONVERT_LOCAL_REGISTRY_SERVER/g" ${CP4BA_PATTERN_FILE_TMP}
        ${SED_COMMAND} "/imageCredentials:/{n;s/registry:.*/registry: "${CONVERT_LOCAL_REGISTRY_SERVER}"/;}" ${CP4BA_PATTERN_FILE_TMP}
    ...
    for object_name in "${object_array[@]}"
    do
        containsObjectStore "$object_name" "${CP4BA_PATTERN_FILE_TMP}"
        if (( ${#os_index_array[@]} > 1 ));then
            ((index_array_temp=${#os_index_array[@]}-1))
            ...
            do
                ...
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.datasource_configuration.dc_os_datasources.[$index_os]
            done
        fi
        containsInitObjectStore "$object_name" "${CP4BA_PATTERN_FILE_TMP}"
        if (( ${#os_index_array[@]} > 1 ));then
            ((index_array_temp=${#os_index_array[@]}-1))
            for ((j=0;j<${index_array_temp};j++))
            do
                ((index_os=${os_index_array[$j]}-j))
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.initialize_configuration.ic_obj_store_creation.object_stores.[$index_os]
            done
        fi
    done
    ...
    containsInitLDAPGroups "${CP4BA_PATTERN_FILE_TMP}"
    if (( ${#ldap_groups_index_array[@]} > 1 ));then
        ((index_array_temp=${#ldap_groups_index_array[@]}-1))
        for ((j=0;j<${index_array_temp};j++))
        do
            ((index_os=${ldap_groups_index_array[$j]}-j))
            ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.initialize_configuration.ic_ldap_creation.ic_ldap_admins_groups_name.[$index_os]
        done

    fi

    containsInitLDAPUsers "${CP4BA_PATTERN_FILE_TMP}"
     if (( ${#ldap_users_index_array[@]} > 1 ));then
        ((index_array_temp=${#ldap_users_index_array[@]}-1))
        for ((j=0;j<${index_array_temp};j++))
        do
            ((index_os=${ldap_users_index_array[$j]}-j))
            ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.initialize_configuration.ic_ldap_creation.ic_ldap_admin_user_name.[$index_os]
        done
    ...
    for object_name in "${baw_name_array[@]}"
    do
        containsBAWInstance "$object_name" "${CP4BA_PATTERN_FILE_TMP}"
        if (( ${#baw_index_array[@]} > 1 ));then
            ((index_array_temp=${#baw_index_array[@]}-1))
            for ((j=0;j<${index_array_temp};j++))
            do
                ((index_os=${baw_index_array[$j]}-j))
                ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.baw_configuration.[$index_os]
            done
        fi
    done

    containsAEInstance "${CP4BA_PATTERN_FILE_TMP}"
     if (( ${#ae_index_array[@]} > 1 ));then
        ((index_array_temp=${#ae_index_array[@]}-1))
        for ((j=0;j<${index_array_temp};j++))
        do
            ((index_os=${ae_index_array[$j]}-j))
            ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.application_engine_configuration.[$index_os]
        done
    fi

    containsICNRepos "${CP4BA_PATTERN_FILE_TMP}"
     if (( ${#icn_repo_index_array[@]} > 1 ));then
        ((index_array_temp=${#icn_repo_index_array[@]}-1))
        for ((j=0;j<${index_array_temp};j++))
        do
            ((index_os=${icn_repo_index_array[$j]}-j))
            ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.initialize_configuration.ic_icn_init_info.icn_repos.[$index_os]
        done
    fi

    containsICNDesktop "${CP4BA_PATTERN_FILE_TMP}"
     if (( ${#icn_desktop_index_array[@]} > 1 ));then
        ((index_array_temp=${#icn_desktop_index_array[@]}-1))
        for ((j=0;j<${index_array_temp};j++))
        do
            ((index_os=${icn_desktop_index_array[$j]}-j))
            ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.initialize_configuration.ic_icn_init_info.icn_desktop.[$index_os]
        done
    fi

    containsTenantDB "${CP4BA_PATTERN_FILE_TMP}"
     if (( ${#tenant_db_index_array[@]} > 1 ));then
        ((index_array_temp=${#tenant_db_index_array[@]}-1))
        for ((j=0;j<${index_array_temp};j++))
        do
            ((index_os=${tenant_db_index_array[$j]}-j))
            ${YQ_CMD} d -i ${CP4BA_PATTERN_FILE_TMP} spec.datasource_configuration.dc_ca_datasource.tenant_databases.[$index_os]
        done
    fi
    ...
    if [[ "$DEPLOYMENT_TYPE" == "demo" && "$INSTALLATION_TYPE" == "new" && ("$SCRIPT_MODE" != "review" || "$SCRIPT_MODE" != "OLM") ]];then
        ${CLI_CMD} delete -f ${CP4BA_PATTERN_FILE_BAK} >/dev/null 2>&1
        sleep 5
        printf "\n"
        echo -e "\x1B[1mInstalling the selected Cloud Pak capability...\x1B[0m"

        APPLY_CONTENT_CMD="${CLI_CMD} apply -f ${CP4BA_PATTERN_FILE_TMP}"
    ...
    ${COPY_CMD} -rf ${CP4BA_PATTERN_FILE_TMP} ${CP4BA_PATTERN_FILE_BAK}
    echo -e "\x1B[1mThe custom resource file used is: \"${CP4BA_PATTERN_FILE_BAK}\"\x1B[0m"
...
function prepare_pattern_file(){
    ...
    FOUNDATION_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_foundation.yaml


    CONTENT_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_content.yaml
    CONTENT_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_content_tmp.yaml
    CONTENT_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_content.yaml

    APPLICATION_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_application.yaml
    APPLICATION_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_application_tmp.yaml
    APPLICATION_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_application.yaml

    DECISIONS_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_decisions.yaml
    DECISIONS_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_decisions_tmp.yaml
    DECISIONS_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_decisions.yaml

    ADS_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_decisions_ads.yaml
    ADS_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_decisions_ads_tmp.yaml
    ADS_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_decisions_ads.yaml

    ARIA_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_document_processing.yaml
    ARIA_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_document_processing_tmp.yaml
    ARIA_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_document_processing.yaml
    ...
    # if [ -f "$CP4BA_PATTERN_FILE_BAK" ]; then
    #     ${COPY_CMD} -rf "${CP4BA_PATTERN_FILE_BAK}" "${CP4BA_EXISTING_BAK}"
    #     ${YQ_CMD} d -i ${CP4BA_EXISTING_BAK} spec.shared_configuration
    # else
    #     ${COPY_CMD} -rf "${FOUNDATION_PATTERN_FILE}" "${CP4BA_PATTERN_FILE_BAK}"
    # fi
    ${COPY_CMD} -rf "${FOUNDATION_PATTERN_FILE}" "${CP4BA_PATTERN_FILE_TMP}"
    if [[ "$DEPLOYMENT_TYPE" == "demo" ]];then
        WORKFLOW_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow.yaml
        WORKFLOW_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_tmp.yaml
        WORKFLOW_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow.yaml

        # WORKSTREAMS_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workstreams.yaml
        # WORKSTREAMS_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workstreams_tmp.yaml
        # WORKSTREAMS_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workstreams.yaml

        WW_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow-workstreams.yaml
        WW_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow-workstreams_tmp.yaml
        WW_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow-workstreams.yaml
        ...
        #     WORKFLOW_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow.yaml
        # else
        #     WORKFLOW_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow-workstreams.yaml
        #     WORKFLOW_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow-workstreams_tmp.yaml
        #     WORKFLOW_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow-workstreams.yaml
        # fi
    elif [[ "$DEPLOYMENT_TYPE" == "enterprise" ]]
    then
        WORKFLOW_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow.yaml
        ...
        WORKFLOW_AUTHOR_PATTERN_FILE=${PARENT_DIR}/descriptors/patterns/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_authoring.yaml
        WORKFLOW_AUTHOR_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_authoring_tmp.yaml
        WORKFLOW_AUTHOR_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_authoring.yaml

        # merge workflow with workstreams templat for workflow-workstreams in 4Q
        ${YQ_CMD} m -a -M ${WORKFLOW_PATTERN_FILE} ${WORKSTREAMS_PATTERN_FILE} > /tmp/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_workstreams.yaml
        WW_PATTERN_FILE=/tmp/ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_workstreams.yaml
        ...
        WW_PATTERN_FILE_TMP=$TEMP_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_workstreams_tmp.yaml
        WW_PATTERN_FILE_BAK=$BAK_FOLDER/.ibm_cp4ba_cr_${DEPLOY_TYPE_IN_FILE_NAME}_workflow_workstreams.yaml
