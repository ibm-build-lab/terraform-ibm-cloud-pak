...
while true;
  ...
  if [ $is_complete_lifecycleservice = false ] && [ `oc get lifecycleservice -A --output=json | jq -c -r '.items[].status.conditions[] | select( .type | contains( "Lifecycle Service Ready")) | .reason'` == "ready"]; then
