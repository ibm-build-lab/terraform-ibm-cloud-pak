...
| `project_name`         | The project name is used to name the cluster with the environment name, like this: `{project_name}-{environment}-cluster`<br />It's also used to label the cluster and other resources                                                                                                                                                                                                                                         |                  | Yes      |
| `owner`                | Use your user name or team name. The owner is used to label the cluster and other resources                                                                                                                                                                                                                                                                                                                                    |                  | Yes      |
| `environment`          | The environment name is used to name the cluster with the project name, like this: `{project_name}-{environment}-cluster`                                                                                                                                                                                                                                                                                                      | `dev`            | No       |
...
| `roks_version`         | OpenShift version to install. List all available versions: `ibmcloud ks versions`, there is no need to include the suffix `_OpenShift` the module will append it to install only the specified version of OpenShift.                                                                                                                                                                                                           | `4.5`            | No       |
...
| `private_vlan_number`  | On **IBM Cloud Classic** (`on_vpc` = `false`) this is the private VLAN assigned to your zone. Make it an empty string to select a private unnamed VLAN or a to create new VLAN if there isn't any (i.e. this is the first cluster in the zone). List available VLAN in the zone: `ibmcloud ks vlan ls --zone <datacenter>`, make sure the the VLAN type is `private` and the router begins with `bc`. Use the `ID` or `Number` |                  | No       |
| `public_vlan_number`   | On **IBM Cloud Classic** (`on_vpc` = `false`) this is the public VLAN assigned to your zone. Make it an empty string to select a public unnamed VLAN or a to create new VLAN if there isn't any (i.e. this is the first cluster in the zone). List available VLAN in the zone: `ibmcloud ks vlan ls --zone <datacenter>`, make sure the the VLAN type is `public` and the router begins with `fc`. Use the `ID` or `Number`    |                  | No       |
| `vpc_zone_names`       | On **IBM Cloud VPC Gen 2** (`on_vpc` = `true`) this is the array with the sub-zones in the region to create the workers groups. List all the zones with: `ibmcloud ks zone ls --provider vpc-gen2`. Example: `["us-south-1", "us-south-2", "us-south-3"]`                                                                                                                                                                      | `["us-south-1"]` | No       |
