...
Add a `module` block to provision the [roks](https://github.com/ibm-build-lab/terraform-ibm-cloud-pak/tree/main/modules/roks#building-a-new-roks-cluster) module. Set `source` to `github.com/ibm-build-lab/terraform-ibm-cloud-pak.git//modules/roks`. Then pass the input parameters depending on the infrastructure to deploy the cluster:
