...
bash scripts/pod-status-check.sh ibm-operator-catalog openshift-marketplace
...
bash scripts/bedrock-pod-status-check.sh operand-deployment-lifecycle-manager ${local.operator_namespace}
...
bash scripts/bedrock-pod-status-check.sh ibm-namespace-scope-operator ${local.operator_namespace}

echo "checking status of ibm-common-service-operator"
bash scripts/bedrock-pod-status-check.sh ibm-common-service-operator ${local.operator_namespace}
...
bash scripts/check-cr-status.sh Ibmcpd ibmcpd-cr ${var.cpd_namespace} controlPlaneStatus; if [ $? -ne 0 ] ; then echo \"CPD control plane failed to install\" ; exit 1 ; fi
