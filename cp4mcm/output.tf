...
output "user" {
  ...
  value = var.enable && length(data.external.kubectl_get_mcm_admin_username) > 0 ? data.external.kubectl_get_mcm_admin_username.0.result.username : ""
}
...
output "password" {
  ...
  value = var.enable && length(data.external.kubectl_get_mcm_admin_password) > 0 ? data.external.kubectl_get_mcm_admin_password.0.result.password : ""
}
