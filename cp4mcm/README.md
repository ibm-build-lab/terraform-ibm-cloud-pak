...
**Module Source**: `github.com/ibm-pett/terraform-ibm-cloud-pak/cp4mcm`
...
To build the cluster in your code, use the ROKS module, pointing it with `source` to the location of this module (`github.com/ibm-pett/terraform-ibm-cloud-pak/roks`). Then pass the input parameters with the cluster specification required to run MCM.
...
Use the `module` block assigning the `source` parameter to the location of this module, either local (i.e. `../cp4mcm`) or remote (`github.com/ibm-pett/terraform-ibm-cloud-pak/cp4mcm`). Then pass the input parameters required to install Cloud Pak for Multi Cloud Management.
