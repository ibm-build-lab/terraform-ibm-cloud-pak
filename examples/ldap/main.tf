...
provisioner "file" {
  source      = "../../modules/ldap/files/DB2_AWSE_Restricted_Activation_11.1.zip"
  destination = "/tmp/DB2_AWSE_Restricted_Activation_11.1.zip"
}

provisioner "file" {
  source      = "../../modules/ldap/files/sds64-premium-feature-act-pkg.zip"
  destination = "/tmp/sds64-premium-feature-act-pkg.zip"
}

provisioner "file" {
  source      = "../../modules/ldap/files/cp.ldif"
  destination = "/tmp/cp.ldif"
}

provisioner "file" {
  source      = "../../modules/ldap/files/db2server-V11.1.rsp"
