...
echo "Creating namespace ${CP4BA_NAMESPACE}"
kubectl create namesapce ${CP4BA_NAMESPACE} --dry-run=client -o yaml | kubectl apply -f
kubectl config set-context --current --namespace=${CP4BA_NAMESPACE}

# Create the Operator catalog source
echo "Creating Catalog Sources ..."

kubectl apply -f -<<EOF
${CP4BA_CATALOGSOURCE_CONTENT}
EOF
...
#kubectl create namespace cp4ba-project --dry-run=client -o yaml | kubectl apply -f -
#
...
#create_secret admin-registrykey cp4ba-project
...
#sleep 180
#
#echo "Deploying Cloud Pak for Automation Subscription ${CP4BA_SUBSCRIPTION}"
#echo "${CP4BA_SUBSCRIPTION}" | oc apply -f -
#
